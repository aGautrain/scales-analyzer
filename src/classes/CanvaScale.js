...
class Canvas {
  ...
  static whiteNoteWidth = 60
  static whiteNoteHeight = 200
  static blackNoteWidth = 30
  static blackNoteHeight = 120
  ...
  drawWhiteKey(offset, width, height, highlighted, precededByBlackKey, followedByBlackKey) {
  ...
  drawOctave(nbOctaves = 2, offset = 0, chord = undefined, drawn = undefined) {
    ...
    Scale.chromaticScale.forEach((notes, index) => {
      ...
      if (notes.every((note) => note.alteration)) {
        this.drawBlackKey(offset + whiteKeyIndex * Canvas.whiteNoteWidth - Canvas.blackNoteWidth / 2, Canvas.blackNoteWidth, Canvas.blackNoteHeight, highlighted)
      } else {

        const precededByBlackKey = Scale.diatonicToneIsPrecededByNonDiatonicSemiTone(notes[0].note);
        const followedByBlackKey = Scale.diatonicToneIsFollowedByNonDiatonicSemiTone(notes[0].note);

        this.drawWhiteKey(offset + whiteKeyIndex * Canvas.whiteNoteWidth, Canvas.whiteNoteWidth, Canvas.whiteNoteHeight, highlighted, precededByBlackKey, followedByBlackKey)
        currentOctaveWidth += Canvas.whiteNoteWidth
        whiteKeyIndex++
      }
